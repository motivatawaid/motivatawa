...
@section('desc', 'Di halaman ini anda bisa edit news.')
...
<script>
    ...
    // Set initial value if old input exists, otherwise use item article
    @if(old('article'))
        quill.root.innerHTML = @json(old('article'));
        document.getElementById('article-hidden').value = @json(old('article'));
    @else
        quill.root.innerHTML = @json($item->article ?? '');
        document.getElementById('article-hidden').value = @json($item->article ?? '');
    @endif
</script>
@endpush